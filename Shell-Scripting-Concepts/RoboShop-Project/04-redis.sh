...
Function $? "Disable Redis module"

dnf module enable redis:7 -y &>> $LOGS_FILE
Function $? "Enable Redis module"

dnf install redis -y &>> $LOGS_FILE
Function $? " Install Redis Module" 

sed -i -e 's/127.0.0.1/0.0.0.0/g'  -e '/protected-mode' c 'protected-mode  no' /etc/redis/redis.conf
Function $? "Update Redis configuration and allowed thr connection"

systemctl enable redis &>> $LOGS_FILE
Function $? "Enable Redis service to start on boot"

systemctl start redis &>> $LOGS_FILE
Function $? "Start Redis service" 
